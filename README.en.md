...
`codex-auto-resume` v1.5.3 retains the v1.5.0 all-workspace preflight for every user turn, automatic resume turn, and subagent trigger turn, including questions and non-Git work. Its registration key remains `actual_thread_id + task_id + workspace_root`, covering Git roots, ordinary directories, and per-thread managed directories. The shared daemon still starts on demand after an eligible registration or reuse. Every Windows rate-limit probe now uses a hidden, kill-on-close Job Object plus a PID and creation-identity descendant snapshot fallback. Cleanup terminates the Job first, applies hidden tree and identity-safe process termination, waits for the root and captured descendants to drain, and closes the Job handle last without replacing an earlier RPC error. POSIX probes retain new-session TERM-to-KILL cleanup. When an access-denied Windows lock probe finds that the lock disappeared before it could be read, acquisition retries; daemon and watchdog locks still use PID plus process identity and preserve every other genuine permission failure, and publish a verified initial daemon heartbeat before the first scan so slow scans do not fail startup handshakes. Leaf-first resume and `billing_policy=included_only` remain unchanged.
