...
`codex-auto-resume` v1.5.1 retains the v1.5.0 all-workspace preflight for every user turn, automatic resume turn, and subagent trigger turn, including questions and non-Git work. Its registration key remains `actual_thread_id + task_id + workspace_root`, covering Git roots, ordinary directories, and per-thread managed directories. The shared daemon still starts on demand after an eligible registration or reuse. On Windows, rate-limit probes and their descendants now run in hidden process groups and the full tree is reaped on success, malformed output, timeout, and startup failure. Daemon and watchdog locks use PID plus process identity, preserve genuine permission failures, and publish a verified initial daemon heartbeat before the first scan so slow scans do not fail startup handshakes. Leaf-first resume and `billing_policy=included_only` remain unchanged.
