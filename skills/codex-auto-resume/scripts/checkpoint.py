...
from auto_resume.watch import (_advance_lineage, _project_lock, _publish_child_terminal_locked,
                               _snapshot_fields, _stage_child_handoff)
from auto_resume.state import FileLock
from auto_resume.workspace import workspace_from_job
...
def main(argv=None):
    ...
    with FileLock(_project_lock(job, args.codex_home), timeout=10):
        ...
        if internal_child:
            job = update_job(job_path, lambda value: value.update(**_snapshot_fields(snapshot)))
